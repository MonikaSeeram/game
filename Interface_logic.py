...
def gameboard_player(game_board, marker1, marker2):
    global button
    button = []
    for i in range(3):
        m = 3 + i
        ...
        for j in range(3):
            n = j
            button[i].append(j)
            get_t = partial(get_text, i, j, game_board, marker1, marker2)
...
def get_text_pc(i, j,gb,marker1, marker2):
    ...
    if winner(board, "X"):
        gb.destroy()
        x = False
        box = messagebox.showinfo("Winner", "Player won the match")# displaying the message through message box
    elif winner(board, "O"):
        gb.destroy()
        x = False
...
def play():
    ...
    head = Label(welcome_frame, text="--- WELCOME TO TIC-TAC-TOE ---",
                 bg='light yellow', fg='black',
                 font=('', 20 , 'bold')).pack(padx=10, pady=40,side='top') #label adds text to the background


    B1 = Button(welcome_frame, text="Single Player", command=with_pc,
                bg='tan', fg='black',
                font=('', 18, 'italic'),
                ).pack(padx=12, pady=12,side='top')

    B2 = Button(welcome_frame, text="Multi Player", command=with_hum, bg='tan', fg='black',
                font=('', 18, 'italic'),
                ).pack(padx=12, pady=12,side='top')

    B3 = Button(welcome_frame, text="Exit", command=menu.quit, bg='tan', fg='black',
                font=('', 18, 'italic'),
                ).pack(padx=12, pady=12,side='top')
    ...
    menu.mainloop() #menu.mainloop is a method on the main window which we execute when we want to run our application.
